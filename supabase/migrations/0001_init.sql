...
select storage.create_bucket('bids', false);

-- Storage policies: allow users to access only their own paths under users/{uid}/...
create policy if not exists "storage_bids_read_own" on storage.objects
  for select using (
    bucket_id = 'bids' and (
      owner = auth.uid() or name like ('users/' || auth.uid()::text || '/%')
    )
  );

create policy if not exists "storage_bids_write_own" on storage.objects
  for insert with check (
    bucket_id = 'bids' and name like ('users/' || auth.uid()::text || '/%')
  );

create policy if not exists "storage_bids_update_own" on storage.objects
  for update using (
    bucket_id = 'bids' and owner = auth.uid()
  ) with check (
    bucket_id = 'bids' and owner = auth.uid()
  );

create policy if not exists "storage_bids_delete_own" on storage.objects
  for delete using (
    bucket_id = 'bids' and owner = auth.uid()
  );
